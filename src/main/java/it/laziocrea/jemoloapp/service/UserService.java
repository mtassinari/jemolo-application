...
public class UserService {
    ...
    public User createUser(UserDTO userDTO) {
        ...
        String pwd = RandomUtil.generatePassword();
        log.debug("password genereta: {}",pwd);
        // String encryptedPassword = passwordEncoder.encode(RandomUtil.generatePassword());
        String encryptedPassword = passwordEncoder.encode(pwd);
        // String encryptedPassword = passwordEncoder.encode("rooster2020");
    ...
    public Page<UserDTO> getAllManagedUsers(Pageable pageable) {
    	...
    	// return userRepository.findAllNotAdmin(pageable).map(UserDTO::new);
    }
    
    @Transactional(readOnly = true)
    public Page<UserDTO> getAllManagedNotAdminUsers(Pageable pageable) {
        // return userRepository.findAllByLoginNot(pageable, Constants.ANONYMOUS_USER).map(UserDTO::new);
    	return userRepository.findAllNotAdmin(pageable).map(UserDTO::new);
    }
    ...
    /**
     * Not admin users should be automatically disabled after 15 days.
     * <p>
     * This is scheduled to get fired everyday, at 01:00 (am).
     */
    @Scheduled(cron = "0 0 1 * * ?")
    ...
    public void disableUser() {
        userRepository
            .findAllByActivatedIsTrueAndCreatedDateBefore(Instant.now().minus(15, ChronoUnit.DAYS))
            .forEach(user -> {
                boolean isAdmin = false;
            	Set<Authority> authorities = user.getAuthorities();
            	for (Iterator<Authority> iterator = authorities.iterator(); iterator.hasNext();) {
					Authority authority = (Authority) iterator.next();
					if (authority.getName().equals("ROLE_ADMIN") || authority.getName().equals("ROLE_SUPERADMIN")) {
						isAdmin = true;
