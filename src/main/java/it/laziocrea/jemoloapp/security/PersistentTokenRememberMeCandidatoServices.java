 ...
 */
@Service("candidatoRemeberMeService")
public class PersistentTokenRememberMeCandidatoServices extends
    AbstractRememberMeServices {

    private final Logger log = LoggerFactory.getLogger(PersistentTokenRememberMeCandidatoServices.class);
    ...
    public PersistentTokenRememberMeCandidatoServices(JHipsterProperties jHipsterProperties,
            DomainCandidatoDetailsService candidatoDetailsService,
            PersistentCandidatoTokenRepository persistentTokenRepository, CandidatoRepository candidatoRepository) {

        super(jHipsterProperties.getSecurity().getRememberMe().getKey(), candidatoDetailsService);
    ...
    protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request,
        ...
        synchronized (this) { // prevent 2 authentication requests from the same user in parallel
            ...
            if (login == null) {
                PersistentCandidatoToken token = getPersistentToken(cookieTokens);
                login = token.getCandidato().getLogin();
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication
        ...
        log.debug("Creating new persistent login for candidato {}", login);
        PersistentCandidatoToken token = candidatoRepository.findOneByLogin(login).map(u -> {
        	PersistentCandidatoToken t = new PersistentCandidatoToken();
            ...
            return t;
        }).orElseThrow(() -> new UsernameNotFoundException("Candidato " + login + " was not found in the database"));
        try {
            persistentTokenRepository.saveAndFlush(token);
            addCookie(token, request, response);
        } catch (DataAccessException e) {
     ...
     */
    private PersistentCandidatoToken getPersistentToken(String[] cookieTokens) {
        ...
        Optional<PersistentCandidatoToken> optionalToken = persistentTokenRepository.findById(presentedSeries);
        if (!optionalToken.isPresent()) {
            // No series match, so we can't authenticate using this cookie
            throw new RememberMeAuthenticationException("No persistent token found for series id: " + presentedSeries);
        }
        PersistentCandidatoToken token = optionalToken.get();
    ...
    private void addCookie(PersistentCandidatoToken token, HttpServletRequest request, HttpServletResponse response) {
        setCookie(
