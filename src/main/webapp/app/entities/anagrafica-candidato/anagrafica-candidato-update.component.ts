...
export class AnagraficaCandidatoUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.candidatoService
      ...
      .subscribe(
        (res: ICandidato[]) => {
          if (!this.editForm.get('candidatoId').value) {
            this.candidatoes = res;
          } else {
            this.candidatoService
              ...
              .subscribe(
                (subRes: ICandidato) => (this.candidatoes = [subRes].concat(res)),
  ...
  save() {
    ...
    if (anagraficaCandidato.id !== undefined) {
      ...
      this.subscribeToSaveResponse(this.anagraficaCandidatoService.createV2(anagraficaCandidato, this.files));
    }
  }
