...
export class IscrittoService {
  ...
  public resourceUrl2 = SERVER_API_URL + 'api/anagrafica-candidatoes2';
  public iscrittiUrl = SERVER_API_URL + 'api/ricerca-iscritti';
  public parametriRicerca: {[k: string]: any}; // = {};
  ...
  find2(id: number): Observable<EntityResponseType> {
    return this.http
      .get<IAnagraficaCandidato>(`${this.resourceUrl2}/${id}`, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IAnagraficaCandidato[]>(this.iscrittiUrl, { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  }

  downloadAllExcel(req?: any): Observable<HttpResponse<Blob>> {
    let headers = new HttpHeaders();
    const options = createRequestOption(req);
    headers = headers.append('Accept', 'application/vnd.ms-excel');
    return this.http.get(`${this.iscrittiUrl}/excel-download`, { headers, params: options, observe: 'response', responseType: 'blob' });
  }
